...
import { $, $$ } from './dom.js';
import { api, backendUrl } from './tmdb.js';
  ...
  try {
    ...
    data.links.forEach((l) => {
      ...
      btn.addEventListener('click', () => {
        closeQualityModal();
        playStream(l.proxiedUrl, l.ext, { fid, title });
      });
...
export async function playStream(url, ext, subCtx = {}) {
  // On Android, hand the stream to the native ExoPlayer (smooth TV playback +
  // D-pad controls). Subtitles are fetched natively from /api/subtitles by fid.
  if (window.__hasNativeBridge && typeof window.__nativePlay === 'function') {
    if (window.__nativePlay(url, ext, subCtx.title || '', subCtx.fid || '')) return;
  }
